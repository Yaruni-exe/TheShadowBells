...
#include <raymath.h>
#include <map>
#include "../enemys/SpecialVampire.h"
    ...
    spawned_enemies_.erase(std::remove_if(spawned_enemies_.begin(), spawned_enemies_.end(),
        [&counts](const std::weak_ptr<enemy::Enemy_Base_Class>& e) {
            if (auto locked_enemy = e.lock()) {
                if (auto vampire = std::dynamic_pointer_cast<enemy::SpecialVampire>(locked_enemy)) {
                    counts[enemy::EnemyType::SPECIAL_VAMPIRE]++;
                }
                return false;
            }
            return true;
                        ...
                        if (valid_pos_found) {
                            ...
                            switch (spawn_info.type) {
                                case enemy::EnemyType::STANDARD_VAMPIRE:
                                    ...
                                    break;
                                case enemy::EnemyType::SPECIAL_VAMPIRE:
                                    new_enemy_raw = new enemy::SpecialVampire(spawn_pos);
                                    break;
                            }
...
{
    return new enemy::SpecialVampire(position);
}
